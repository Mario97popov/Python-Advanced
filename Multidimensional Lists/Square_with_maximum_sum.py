...
def get_best_submatrix_sum(matrix, submatrix_size):
    ...
    for row_index in range(len(matrix) - sumatrix_size + 1):
        for col_index in range(len(matrix[row_index]) - sumatrix_size + 1):
            current_sum = get_sum_of_submatrix(matrix, row_index, col_index, sumatrix_size)
...
matrix = read_matrix()
sumatrix_size = 2
coordinates = get_best_submatrix_sum(matrix, sumatrix_size)
print_result(coordinates, sumatrix_size)
